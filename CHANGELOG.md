...
- 页面文案去重：「当前后端」×3、「统一朗读」×4、访问规则×3 等重复表述各保留一处；删去介绍界面自身机制的元注释与重复 kicker。
- 修复渲染重复：一次刷新不再重复渲染就绪状态与状态标签。
- 保存与上传收紧：保存后信任响应中的就绪信息，不再全量重拉；音色样本上传不再重复提交元数据；表单事件改为每元素单一绑定；桌面与移动端诊断按钮文案对齐。
- 断行兜底：运行条选项不折行，折叠条右侧指示不被压扁，长音色标识不再被裁切。
- 备注：`pages_api.py` 的 `save_config` 响应新增 `readiness` 字段，供前端保存后直接刷新就绪状态。

## 0.12.6 - 2026-09-16

### 变更

- 支持核接管「一键读取当前配置 / 一键固化到插件」：一键读取/固化：native_view/snapshot native_value；固化写入 data_dir/config.json（先备份、原子落盘、失败回滚），并同步刷新运行时。

## 0.12.5 - 2026-09-14

### 变更

- 11 个布尔开关（LLM 判断适读 / 失败回退文件 / URL 改写 / 句界兜底 / 外部 TTS API / AI 导演及其 3 个子项 / 情绪路由 / 授权确认）改用 series.ui 统一滑块开关。
- 语音后端二选一、触发模式二选一仍为单选（radio），语义不变。
...
- 语音后端选择升级为常驻运行条（所有分区跟随），Hero 压缩为一行，保存 / 诊断在桌面 sticky、移动端固定底栏。
- 四个任务页签保留；总览聚合后端、音色库、试听入口与关键开关摘要，高级配置进入折叠区。
- 42 项配置、音色 CRUD、试听、访问控制与整份保存语义保持不变。
...
- 页面改为总览 / 后端与音色 / 对话与交付 / 接口与权限四个任务页签，语音后端选择变为常驻作用域切换器。
- 修复试听栏被挤压的问题（输入框从约 31px 恢复为整行可用）。
- 访问控制改为规则表 + 编辑抽屉并实时统计人数；情绪路由并入 AI 风格导演。
- 术语汉化（访问密钥、接口地址、模型服务商标识、完整会话标识）并接入底部安全区。

## 0.12.1 - 2026-09-13

### 变更

- 语音页面后端选择前置、移动端 Hero 压缩、分区导航 sticky；补齐共享 UI 布局组件。

## 0.12.0 - 2026-09-13

### 新增

- 全面接入 `series.ui@1.0` Glass Aurora 组件库，统一音色卡片、上传区、波形、试听台和流程状态，保留语音业务专属布局。
...
- 将 managed `series.webui@2.0` 管理面扩展到音色创建、更新、删除、默认映射、情绪映射、参考音频上传、常用 TTS/AI 配置与连接测试；继续复用现有 `VoiceStore`、音频校验和合成服务。
- 新增文件制品、动作风险元数据、双层面板与脱敏错误投影；核接管模式下保留 `preview_voice` 并继续返回短时 `audio.data`。

## 0.10.0 - 2026-09-12

### 新增

- 接入 `series.module@1.0` 与 `series.webui@2.0`；核统一接管时可选择音色、填写文本并生成短时 WAV 制品试听。
- 语音导演辅助调用接入统一 `fast` 模型角色。
...
- 新增只读 `series.webui@1.0` 音色总览面板：核独立 WebUI 可查看音色、默认映射与就绪状态；现有 `pages/settings` 与全部 standalone 路由继续保留。
...
- 设置页在 Bridge 初始化前绑定交互，初始化超时会显示可重试错误，不再让控件静默失效。
- 补充键盘焦点、触屏 hover 隔离、播放态波形和 reduced-motion 支持。

## 0.8.10 - 2026-08-19

- 交付边界增强：当其他插件已经在结果链放入 `Record`/明确音频组件时，声会记录
  `external_audio_present` 并跳过自动 TTS；普通 `File` 仍按附件处理，避免图片或文档被误判为音频。
- 继续消费言的交付计划与取消令牌；外部插件通过平台对象直接发送的内容不被声猜测或重复生成。

## 0.8.9 - 2026-08-10

- 修复外部 API 自诊断固定访问 `127.0.0.1` 且未区分 Token 缺失、服务启动竞态和连接失败的问题：诊断现在使用当前配置的 Bearer Token，等待本插件监听实例就绪，并按实际监听地址访问；外部 API 的鉴权要求保持不变。
- 外部 API 服务仅在端口成功绑定后报告就绪；绑定失败、连接失败和请求超时提供独立诊断原因码，并补充启动状态与 Pages 自诊断回归测试。

## 0.8.8 - 2026-08-09

- 系列诊断提供方补齐统一自声明字段，可由“核”动态发现并独立清空。
- README 的访问控制示例移除具体平台账号和会话号，统一使用语义占位符；运行时配置与访问控制行为不变。
...
- 修复 `voice_hub_speak` 已直接发送语音后，模型把 `<audio already sent>` 或工具完成状态复述成用户可见文本的问题；清理仅对已成功发送语音且最终结果为纯内部占位符的事件生效，避免误删正常回复。

## 0.8.5 - 2026-08-03

### 变更

- 统一“凝心溯溪”系列页面色板和中文栏目，MiMo/AstrBot 专属设置、概率设置、外部 API 和 AI 导演按当前开关收起。
- 情绪内部值保持兼容，页面显示中文情绪名称；音色 ID 和情绪动态属性增加安全转义。
- 配置、AI 服务商和 TTS 服务商在同一轮并发读取，TTS 输出继续按 `voice.audio_output@1.0` PCM WAV 契约校验。

### 测试

- 补充页面中文化、动态属性转义和并发服务商加载回归测试。

## 0.8.4 - 2026-08-02

### 变更

- Pages 重构为通用“声 / 统一语音中心”：后端选择独立成区，统一朗读工具、触发策略、结构化分段、等待、取消和输出清理归入跨后端区域。
- MiMo Key、模型、AI 导演、情绪路由、旧配置迁移、音色库和试听工作台仅在 MiMo 后端显示；AstrBot 模式只展示对应 TTS 提供商设置，工作台状态与连接诊断随当前后端切换。
- 配置 schema 将语音后端提升为首要选项，并明确标注 MiMo/AstrBot 专属字段；现有配置键和 Pages 保存 API 保持兼容。

### 验证说明

- 单元测试、静态检查、JavaScript 语法、HTML ID 和响应式 CSS 规则均已验证；真实 Playwright 截图验证因本机 Chrome 运行时阻塞未完成。

## 0.8.3 - 2026-08-02

### 变更

- 统一模型可见的 TTS 工具为 `voice_hub_speak`，后端按插件配置在 MiMo 与 AstrBot TTS 提供商之间选择；兼容原有 `text` 参数，并新增可选的结构化 `segments`（逐段支持文本、情绪、音色和风格覆盖）。
- 朗读分段改为显式空行/编号段落优先，单段超长时才按句界兜底；不再跨段合并以满足分段上限，并保留原有逐段延迟与取消语义。
- 移除仓库内未发现外部调用的 `mimo_tts_speak` 模型工具注册，避免同一能力向模型暴露重复工具。

## 0.8.2 - 2026-08-01

### 变更

- 长文本分段新增可调智能等待：第一条立即发送，后续每条按自身 WAV 时长乘“每 1 秒语音等待毫秒数”；无法按时长计算时回退到可调固定等待；等待期间继续响应取消令牌。

## 0.8.1 - 2026-08-01

### 变更

- 为当前衍生版本补充英文版权署名 `qsbb`，同时保留 Justice-ocr 的真实上游版权与致谢。
- 新增长文本语音段间等待：默认 350 毫秒，可在“长文本分段”设置中调整或关闭；等待期间继续响应取消令牌。
- 完善系列诊断日志：捕获范围扩展到 `DEBUG`-`CRITICAL`，单插件内存缓冲上限为 1000 条；自动捕获事件现在保留最长 2000 字符的脱敏日志正文，供“核”点击展开，且继续不读取 AstrBot 全局日志。

- 长文本语音分段取消时只停止尚未发送的语音并把控制权交回当前 LLM，不再由声直接终止整轮事件。

## 0.8.0 - 2026-07-31

### 头像

- 更新凝心溯溪-声 的插件头像为本次发布的 `logo.png`。
...
- 发布元数据与运行时注册的当前维护者统一为凌溪；继续保留 Justice-ocr 的原项目版权与致谢。

### 安全

- 诊断日志与 AstrBot 主日志隔离，写入前会脱敏敏感标识并截断过长内容，不保存聊天正文、令牌或异常堆栈；日志不落盘，插件重载或 AstrBot 重启后自动清空。
- 自动告警仅暴露结构化定位信息；AI 导演调试只记录文本长度，不记录三段正文。清空会轮换诊断流标识，避免新旧实例事件混合。

## 0.7.8 - 2026-07-29

### 修复

- `mimo_tts_speak` 工具链现在登记独立交付令牌，并在合成前后、逐段发送前后检查取消；新消息到达后旧请求停止发送后续语音并清理未交付结果。
- `text_to_speech()` 和 OpenAI 兼容 API 的多段 WAV 改为完整解析与合并，不再把第一段当作完整长音频；格式无效或参数不兼容时明确失败，AstrBot 内置 TTS 可关闭分段重试一次。

### 变更

- 同步统一请求上下文的结构化提示片段 helper，保持六个系列插件的契约副本字节一致；声不生产或消费提示片段。

### 测试

- 新增工具取消、合成后取消、逐段取消、临时文件清理、WAV 完整合并、参数失配、外部 API 错误及畸形提示片段读取回归测试。
