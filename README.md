...
- **OpenAI 兼容外部 API**：开启后插件启动带 Bearer 认证、模型校验、频率限制和输入长度限制的 `POST /v1/audio/speech` 服务；默认仅监听 `127.0.0.1`。Pages 的连接诊断会使用当前配置中的 Token 自调用此接口；未配置 Token 时会直接提示服务未启动，外部客户端仍必须携带同一 Token。
...
声只消费正式结果链和 `conversation_flow.delivery_plan@1.0`，并在合成前、每段完成后、
每段发送前检查取消令牌。若结果链已经包含明确的 `Record`/音频组件，声只记录
`external_audio_present` 并跳过自动 TTS；普通 `File` 不会被当作音频，因此图片、文档等附件
不会误触发这个保护。

第三方插件若直接调用平台或上下文的发送 API，声不会全局拦截、撤回或猜测其交付状态。需要纳入
统一分段和取消语义的发送方应发布交付计划，而不是依赖隐式的加载顺序。
...
页面按任务分区，顶部页签为 `总览 / 后端与音色 / 对话与交付 / 接口与权限`；`语音后端` 是常驻的作用域切换器，移动端首屏即可切换，四个页签的专属设置都跟随它显隐。

页面重点区域：

- `语音后端`：常驻切换 MiMo 或 AstrBot TTS；切换后立即收起无关后端字段。
- `统一朗读与触发`：管理 `voice_hub_speak`、回复模式、概率触发或 LLM 自主决定。
- `分段、等待与取消`：显式段落优先，单段过长才按句界兜底；统一设置逐段等待、取消说明和输出清理。
- `外部语音接口`：复用当前后端并集中管理监听、令牌和限流。
- `MiMo 设置`：仅 MiMo 模式显示 Key、模型、旧配置迁移，以及合并了情绪路由的 AI 风格导演（先定情绪、再定风格），音色库与试听工作台同域展示。
- `AstrBot 设置`：仅 AstrBot 模式显示 TTS 提供商选择，不占用 MiMo 配置区域。

页面采用“凝心溯溪”系列的浅色玻璃（Glass Aurora）样式，栏目、运行状态和内置情绪名称优先显示中文；`happy`、`sad`、`angry`、`neutral` 等内部值保持不变，不影响旧配置。打开或刷新页面时，插件配置、AI 服务商和 TTS 提供商会在同一轮并行读取，单个可选列表读取失败不会阻断主配置展示。
